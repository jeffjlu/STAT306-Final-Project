...
We see that all of our variables except Thigh are significant at the 5 % level. It is slightly surprising that weight and wrist have a negative coefficient because the exploratory data analysis demonstrated that these variables have a positive relationship with bodyfat percentage.

We can examine this further using partial correlations. We calculate partial correlation between wrist bodyfat while adjusting for different combinations of covariates.
...
paste("Partial correlation between wrist and bodyfat", cor(reg_bodyfat$residuals, reg_wrist$residuals))

reg_bodyfat = lm(BodyFat ~ Weight + Forearm + Age + Thigh, data=bodyfat)
reg_wrist =   lm(Wrist   ~ Weight + Forearm + Age + Thigh, data=bodyfat)
cor(reg_bodyfat$residuals, reg_wrist$residuals)

reg_bodyfat = lm(BodyFat ~ Forearm + Age + Thigh, data=bodyfat)
reg_wrist =   lm(Wrist   ~ Forearm + Age + Thigh, data=bodyfat)
cor(reg_bodyfat$residuals, reg_wrist$residuals)

reg_bodyfat = lm(BodyFat ~  Age + Thigh, data=bodyfat)
reg_wrist =   lm(Wrist   ~  Age + Thigh, data=bodyfat)
cor(reg_bodyfat$residuals, reg_wrist$residuals)

reg_bodyfat = lm(BodyFat ~   Thigh, data=bodyfat)
reg_wrist =   lm(Wrist   ~   Thigh, data=bodyfat)
cor(reg_bodyfat$residuals, reg_wrist$residuals)
