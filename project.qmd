...
plot_grid(bodyfat_age, bodyfat_weight, bodyfat_height, bodyfat_abdom,bodyfat_thigh,bodyfat_hip,bodyfat_knee, ncol=3, nrow= 2, scale = 1)
...
We will also calculate some summary statistics to get an idea of the variables in ourdataset.
...
num_nas = data.frame(colSums(is.na(bodyfat)))
num_nas
```
...
Our exploratory data analsysis suggested that a linear model may be approrpriate to explain the relationship between body fat percentage and various physical measurmenets. To address our research question, we would attempt to fit a linear regression model, with body fat percentage as our response and a suitable combination of other variables. We would perform backward selection to determine which variables contain the most useful information to explain the variation in body fat percentage.
...
To further narrow down our options, we can compute Mallows' $C_p$ statistic for each model, treating the model with 8 covariates as our full model. We also tabulate the values for $$R^2$$and Adjusted $$R^2$$.
